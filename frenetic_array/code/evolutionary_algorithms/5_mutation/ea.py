...
class EA:
    ...
    def __log(self, total_generations, epochs, generation):
        ...
        mutation_rates = [individual.mutation_rate.rate for individual in self.population.individuals]

        best_fitness = max(fitnesses)
        average_fitness = sum(fitnesses) / len(fitnesses)
        average_mutation_rate = sum(mutation_rates) / len(mutation_rates)

        format_string = "Total Generations #{}, Epoch #{}, Generation #{}: Average Mutation Rate – {:.4f}, Best Fitness – {:.4f}, Average Fitness – {:.4f}"
        log_string = format_string.format(total_generations, epochs, generation, average_mutation_rate, best_fitness, average_fitness)
